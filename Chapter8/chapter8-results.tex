...
The experimental findings and analysis have confirmed that the designed federated Hybrid HGNN–LSTM architecture is indeed feasible for Trade-Based Money Laundering detection and regulatory compliance in distributed settings. Specifically, the results obtained revealed the capability to achieve stable graph-based fraud detection performance, ensure reliable federated convergence process, control scalability features, and perform proper synchronization in real time in a distributed setting. Furthermore, the designed framework ensured privacy-preserving collaborative learning without needing any centralization of transaction data.
