...
Figure \ref{fig:dfd2_ingestion} illustrates the internal workflow of the Data Ingestion Module. The process begins with the collection of raw transaction records, customer information, and trade-related documents from external financial sources. The acquired data undergoes validation, cleansing, and standardization to remove inconsistencies and prepare it for downstream analytical processing. Finally, the processed transaction events are published to the streaming infrastructure, enabling continuous real-time data availability for graph construction and fraud analysis.
...
Figure \ref{fig:dfd2_graph} presents the detailed workflow of the Graph Processing Module. Streaming transaction events are consumed and transformed into graph representations where entities and relationships are modeled as interconnected nodes and edges. Relevant structural and transactional features are extracted from the generated graph, after which the resulting graph structures are stored within the Memgraph database. This module provides the graph-based representations required for heterogeneous graph learning and fraud detection.
...
Figure \ref{fig:dfd2_detection} depicts the internal operation of the Hybrid Detection Module. Graph embeddings generated through the HGNN branch and temporal embeddings produced by the LSTM branch are combined through feature fusion to create a unified financial representation. The fused feature vector is subsequently analyzed to determine transaction risk levels and classify activities into Clean, Suspicious, or Fraud categories. This module forms the core intelligence engine responsible for identifying complex TBML patterns.
...
Figure \ref{fig:dfd2_federated} illustrates the Federated Learning Module responsible for privacy-preserving collaborative model training. Each participating banking institution performs local model training using its own transaction data and securely transmits encrypted model parameters to the aggregation server. The FedAvg aggregation mechanism combines local updates to generate a global model, which is redistributed to all participating clients for subsequent training rounds. This process enables cross-institutional learning without exposing sensitive customer information.
...
Figure \ref{fig:dfd2_compliance} illustrates the Compliance and Reporting Module. Risk scores generated by the detection framework are analyzed to identify high-risk cases requiring regulatory attention. The module generates Suspicious Transaction Reports (STRs), maintains detailed audit logs, and prepares regulatory reports for compliance review. Human-in-the-loop verification mechanisms further support investigative decision-making before final report submission.
...
Figure \ref{fig:dfd2_dashboard} presents the Notification and Dashboard Module. Risk assessments and fraud alerts generated by the detection framework are visualized through interactive dashboards for analyst review. The module manages alert generation, email notifications, administrative monitoring, and system log maintenance. These capabilities provide investigators and compliance teams with real-time visibility into suspicious activities and system operations.

%========================================================

Collectively, Figures \ref{fig:dfd2_ingestion}--\ref{fig:dfd2_dashboard} provide a detailed Level 2 decomposition of the proposed Federated HGNN-based TBML Detection Framework. The diagrams illustrate how transactional data progresses from ingestion and graph construction to hybrid HGNN-LSTM analysis, federated model optimization, compliance reporting, and real-time alert dissemination. By decomposing the framework into specialized functional modules, the Level 2 DFDs clearly represent the internal data movement, process interactions, and decision-making workflow that enable scalable, privacy-preserving, and real-time Trade-Based Money Laundering detection across distributed financial institutions.
