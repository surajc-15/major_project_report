...
Despite demonstrating effective fraud-detection capability and stable federated analytical performance, the proposed framework is subject to several limitations:

\begin{enumerate}
\item \textbf{Limited Availability of Real-World TBML Datasets:} The experimental evaluation primarily relied on synthetically enriched transactional datasets due to the limited availability of institutionally verified TBML datasets containing real-world laundering annotations. Consequently, certain emerging laundering behaviors, cross-border transaction irregularities, and complex trade-manipulation patterns may not be fully represented within the current evaluation environment.

\item \textbf{Federated Communication and Computational Overhead:} The federated learning workflow introduces additional communication overhead during distributed parameter synchronization and aggregation. As the number of participating institutions increases, synchronization latency, network communication costs, and distributed optimization complexity may increase, potentially affecting large-scale deployment efficiency.

\item \textbf{Absence of Automated Enforcement Mechanisms:} The current implementation focuses on fraud detection, monitoring, and compliance-notification workflows. Although regulator-side alert propagation and investigation support are provided, automated enforcement actions such as transaction blocking, account freezing, document-lock execution, and adaptive compliance-trigger mechanisms are not directly integrated into the operational banking environment.
