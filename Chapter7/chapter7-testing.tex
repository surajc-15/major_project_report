...
The authentication and role-based access control environment was independently validated to ensure secure user authentication, authorization management, JWT token generation, cookie persistence, middleware-driven route protection, credential verification, and institutional role isolation across banking and regulatory dashboard interfaces. The testing workflow additionally verified login-session management, token storage mechanisms, unauthorized route restrictions, and role-specific dashboard rendering across distributed monitoring environments.
...
\begin{table}[H]
\centering
\caption{TC-001: Failed Authentication Session Validation}
\label{tab:auth_failed_test}
\vspace{0.2cm}
\begin{tabular}{|p{4.2cm}|p{9cm}|}
\hline
\textbf{Serial Number of Test Case} & TC-001 \\ \hline
\textbf{Name of Test Case} & User Login and Session Persistence Validation \\ \hline
\textbf{Feature being Tested} & JWT Token Generation and Cookie Storage Mechanism \\ \hline
\textbf{Description} & Validate successful authentication and automatic login session creation after credential verification \\ \hline
\textbf{Sample Input} & Valid username and password credentials \\ \hline
\textbf{Expected Output} & User authenticated successfully and JWT token stored in browser cookies for protected dashboard access \\ \hline
\textbf{Actual Output} & Authentication completed but JWT token was not persisted in cookies, causing automatic session validation failure \\ \hline
\textbf{Remarks} & Failed test \\ \hline
\end{tabular}
\end{table}

As shown in Table~\ref{tab:auth_failed_test}, the initial authentication workflow successfully validated user credentials and generated a JWT access token. However, the generated token was not properly persisted within browser cookies because of a cookie-configuration issue in the authentication middleware. Consequently, the session validation mechanism failed during protected route verification, causing users to be redirected back to the login interface despite successful credential authentication. The test identified shortcomings in cookie storage configuration and session persistence handling that prevented seamless authentication across protected dashboard environments.
...
\begin{table}[H]
\centering
\caption{TC-002: Successful Authentication and JWT Session Validation}
\label{tab:auth_success_test}
\vspace{0.2cm}
\begin{tabular}{|p{4.2cm}|p{9cm}|}
\hline
\textbf{Serial Number of Test Case} & TC-002 \\ \hline
\textbf{Name of Test Case} & User Login and Protected Dashboard Access Validation \\ \hline
\textbf{Feature being Tested} & Authentication, JWT Session Management, and RBAC Authorization \\ \hline
\textbf{Description} & Validate complete login workflow including credential verification, JWT generation, cookie persistence, and protected route authorization \\ \hline
\textbf{Sample Input} & Valid username and password credentials \\ \hline
\textbf{Expected Output} & User authenticated successfully, JWT stored correctly, and role-based dashboard access granted \\ \hline
\textbf{Actual Output} & JWT generated and persisted successfully, session validated correctly, and authorized dashboard rendered without errors \\ \hline
\textbf{Remarks} & Successful test \\ \hline
\end{tabular}
\end{table}

As shown in Table~\ref{tab:auth_success_test}, the authentication workflow was retested after correcting the cookie persistence and session-management configuration. The revised implementation successfully generated and stored JWT tokens within secure browser cookies, enabling middleware-driven session validation across protected routes. The testing workflow verified successful authentication, automatic session restoration, role mapping for \texttt{BANK\_USER} and \texttt{ADMIN} entities, protected API authorization, and secure dashboard rendering. All protected resources remained accessible only to authorized users, thereby confirming the reliability of the authentication and RBAC infrastructure within the proposed framework.
...
\begin{table}[H]
\centering
\caption{TC-003: Unit Testing of Kafka Transaction Streaming}
...
\begin{table}[H]
\centering
\caption{TC-004: Unit Testing of Graph Construction Module}
...
\begin{table}[H]
\centering
\caption{TC-005: Unit Testing of STR Generation Workflow}
...
\begin{table}[H]
\centering
\caption{TC-006: Unit Testing of Notification Workflow}
...
\begin{table}[H]
\centering
\caption{TC-007: Integration Testing of Kafka and Memgraph Pipeline}
...
\begin{table}[H]
\centering
\caption{TC-008: Integration Testing of Fraud Prediction and Dashboard Synchronization}
...
\begin{table}[H]
\centering
\caption{TC-009: Integration Testing of STR Generation Workflow}
...
\begin{table}[H]
\centering
\caption{TC-010: Integration Testing of Authentication and Protected Dashboard Rendering}
\label{tab:auth_dashboard_integration_test}
\vspace{0.2cm}

\begin{tabular}{|p{4.2cm}|p{9cm}|}
\hline
\textbf{Serial Number of Test Case} & TC-010 \\ \hline
...
The detailed round-wise convergence metrics corresponding to the graphs presented in Figure~\ref{fig:banka_progression} are provided in Appendix~\ref{tab:banka_round_metrics}.
...
The detailed round-wise convergence metrics corresponding to the graphs presented in Figure~\ref{fig:bankb_progression} are provided in Appendix~\ref{tab:bankb_round_metrics}.
%--------------------------------------------------------
...
The detailed round-wise convergence metrics corresponding to the graphs presented in Figure~\ref{fig:bankc_progression} are provided in Appendix~\ref{tab:bankc_round_metrics}.
