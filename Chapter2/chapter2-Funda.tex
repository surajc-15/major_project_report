\chapter{Theory and Concepts of the Proposed TBML Detection Framework}
...
\paragraph{\textbf{(i) Over Invoicing}}
...
\paragraph{\textbf{(ii) Under Invoicing}}
...
\paragraph{\textbf{(iii) Multiple Invoicing}}
...
\paragraph{\textbf{(iv) Phantom Shipments}}
...
\paragraph{\textbf{(v) Misrepresentation of Goods}}
...
\subsubsection{B.   Fundamental Concept of GATv2}
...
\subsubsection{C.   Architecture and Working of GATv2}

The overall architecture and workflow of GATv2 are shown in Figure~\ref{fig:gatv2_architecture}.
...
The input node features are transformed using a learnable weight matrix. The feature transformation operation is represented by Equation~\eqref{eq:feature_transform}.
...
For every connected node pair \((i,j)\), the attention score is computed as. The attention coefficient calculation between neighboring nodes is given by Equation~\eqref{eq:attention_score}.
...
The attention scores are normalized using the softmax function. The normalization process that converts raw attention scores into attention coefficients is defined in Equation~\eqref{eq:attention_normalization}.
...
The neighboring node features are aggregated using weighted summation. The aggregation of neighborhood information based on the computed attention coefficients is represented by Equation~\eqref{eq:feature_aggregation}.
...
GATv2 employs multiple attention heads operating in parallel. The multi-head attention mechanism used to capture diverse neighborhood representations is expressed by Equation~\eqref{eq:multihead_attention}.
...
\subsubsection{D.   Advantages of GATv2}
...
\subsubsection{E.   Limitations of GATv2}
...
In the original GAT architecture, the attention mechanism is defined as. The computation of attention scores between connected nodes in the original GAT model is represented by Equation~\eqref{eq:gat_attention}.
...
In GATv2, the order of operations is modified as. The dynamic attention formulation introduced in GATv2 is represented by Equation~\eqref{eq:gatv2_attention}.
...
This comparison highlights the key differences between the original Graph Attention Network (GAT) and Graph Attention Network v2 (GATv2) in terms of attention mechanism, flexibility, expressiveness, and overall performance. The comparative analysis is presented in Table~\ref{tab:gat_vs_gatv2}.
...
\subsubsection{A.   Introduction to LSTM}
...
\subsubsection{B.   Fundamental Concept of LSTM}
...
The mathematical formulation of the input, forget, and output gates in an LSTM cell is given by Equation~\eqref{eq:lstm_gates}.
...
\subsubsection{(ii). Input Node}

The computation of candidate memory information, which represents newly learned information to be incorporated into the memory cell, is defined in Equation~\eqref{eq:candidate_memory}.

\begin{equation}
\tilde{\mathbf{C}}_t = \tanh(\mathbf{X}_t\mathbf{W}_{xc} + \mathbf{H}_{t-1}\mathbf{W}_{hc} + \mathbf{b}_c)
\label{eq:candidate_memory}
\end{equation}
...
\subsubsection{(iii). Memory Cell Internal State}

The update mechanism of the memory cell state, which combines retained past information with newly learned information, is defined in Equation~\eqref{eq:cell_state_update}.

\begin{equation}
\mathbf{C}_t = \mathbf{F}_t \odot \mathbf{C}_{t-1} + \mathbf{I}_t \odot \tilde{\mathbf{C}}_t
\label{eq:cell_state_update}
\end{equation}
...
\subsubsection{(iv). Hidden State}

The computation of the hidden state, which represents the output of the LSTM cell at the current time step, is defined in Equation~\eqref{eq:hidden_state}.
...
\subsubsection{D.   Advantages of LSTM}
...
\subsubsection{E.   Limitations of LSTM}
...
The workflow of the Federated Learning process, including model distribution, local training, parameter aggregation, and global model updates, is illustrated in Figure~\ref{fig:fl_workflow}.
...
The aggregation of locally trained model parameters into a global model is performed using the Federated Averaging (FedAvg) algorithm, as defined in Equation~\eqref{eq:fedavg}.
...
\begin{enumerate}
    \item \textbf{Rendering and Initialization:}
        Initially, dashboard components are rendered within the server environment where transaction summaries, analytical data, and monitoring information are processed before delivery to the client browser. After the interface is loaded within the browser, React hydration enables interactive client-side functionalities including event handling, asynchronous API communication, local state initialization, and real-time dashboard services. This combination improves initial loading performance and enables interactive behavior without full page reloads.

    \item \textbf{Reactive Updates and Synchronization:}
        As transaction states, fraud alerts, and analytical outputs change, React selectively updates only the affected interface components using Virtual DOM reconciliation mechanisms instead of re-rendering the complete dashboard. The framework continuously synchronizes updated transaction information, compliance alerts, and monitoring analytics with the dashboard interface to support real-time fraud visualization and analytical consistency.

    \item \textbf{Resource Cleanup and Unmounting:}
...
\begin{enumerate}
    ...
    \item \textbf{Analytical Visualization Components:}
    ...
    \item \textbf{Role-Based Dashboard Interfaces:}
    ...
    \item \textbf{Interactive Monitoring Services:}
...
The detailed implementation and integration of these visualization technologies within the proposed TBML detection framework are discussed further in the system implementation chapter. The implementation focuses on real-time dashboard synchronization, fraud alert visualization, transaction investigation workflows, and Suspicious Transaction Report (STR) management functionalities.
